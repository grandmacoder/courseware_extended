 ...
 */
class TCCW_CourseProgress extends WP_Widget
{
	/** constructor */
	function __construct()
	{
		...
		$widget_ops = array(
			...
			'description' => __('A widget that shows the current progress TC style of the user through the selected training course.', 'coureseware_extended')
		);
		...
		/* Create the widget. */
		parent::__construct('wpcw_course_progress', __('TCCW Course Progress', 'courseware_extended'), $widget_ops, $control_ops);
	}
	...
	function form($instance)
	{
		...
		if ($instance) {
			...
			$title = __( 'Current User Progress', 'wp-courseware' );
		}
		...
		$optionsList_previous = array(
			'all' 	=> __('All previous modules', 'wp-courseware'),
			'none' 	=> __('None', 'wp-courseware'),
		);
		for ($i = 1; $i <= 20; $i++) {
		...
		$optionsList_next = array(
			'all' 	=> __('All subsequent modules', 'wp-courseware'),
			'none' 	=> __('None', 'wp-courseware'),
		);
		for ($i = 1; $i <= 20; $i++) {
		...
		<p>
			...
			<?php
				$courseList = array(
					'' 			=> __('-- Select a Training Course --', 'wp-courseware'),
					'current' 	=> __("Show User's Current Course", 'wp-courseware'),
				);
			...
			?>
			<br/><small><?php _e('(Required) Choose whether to display a specific course to the user or to display the course associated with the unit that the user is currently viewing.', 'wp-courseware'); ?></small>
		</p>
		...
		<p>
			<b style="display: block; padding-bottom: 3px;"><label for="<?php echo $this->get_field_id('option_toggle_modules'); ?>"><?php _e('Expand/Contract Modules:', 'wp-courseware'); ?></label></b>
			<?php
				echo WPCW_CourseProgress::createDropdown($this->get_field_name('option_toggle_modules'),
					array(
						'expand_all' 				=> __('Expand all modules', 'wp-courseware'),
						'contract_all_but_current' 	=> __('Contract all except current module', 'wp-courseware'),
						'contract_all' 				=> __('Contract all modules', 'wp-courseware'),
					),
				$option_toggle_modules, $this->get_field_id('option_toggle_modules'));
			?>
			<br/><small><?php _e('You can save sidebar space by contracting  modules in the widget to just show the module title.', 'wp-courseware'); ?></small>
		</p>
		...
		<p>
			<b><label for="<?php echo $this->get_field_id('option_module'); ?>"><?php _e('Module:', 'wp-courseware'); ?></label></b>
			...
			<small><?php _e('(Optional) The module number of a module in this course to show specifically (rather than all modules in the course).', 'wp-courseware'); ?></small>
		</p>
		*/ ?>

		<p>
			<b style="display: block; padding-bottom: 3px;"><label for="<?php echo $this->get_field_id('option_show_module_desc'); ?>"><?php _e('More Options:', 'wp-courseware'); ?></label></b>
			<input id="<?php echo $this->get_field_id('option_show_course_title'); ?>" name="<?php echo $this->get_field_name('option_show_course_title'); ?>" type="checkbox" <?php echo $option_show_course_title; ?> /> <?php _e('Show Course Title', 'wp-courseware'); ?><br/>
			<input id="<?php echo $this->get_field_id('option_show_course_desc'); ?>" name="<?php echo $this->get_field_name('option_show_course_desc'); ?>" type="checkbox" <?php echo $option_show_course_desc; ?> /> <?php _e('Show Course Description', 'wp-courseware'); ?><br/>
			<input id="<?php echo $this->get_field_id('option_show_module_desc'); ?>" name="<?php echo $this->get_field_name('option_show_module_desc'); ?>" type="checkbox" <?php echo $option_show_module_desc; ?> /> <?php _e('Show Module Descriptions', 'wp-courseware'); ?><br/>

			<input id="<?php echo $this->get_field_id('option_show_only_on_units'); ?>" name="<?php echo $this->get_field_name('option_show_only_on_units'); ?>" type="checkbox" <?php echo $option_show_only_on_units; ?> /> <?php _e('Only display this widget when showing a course unit', 'wp-courseware'); ?>
		</p>
		<?php
	}
